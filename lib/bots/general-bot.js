    ...
    while (response.stop_reason === 'tool_use' && iterationCount < maxIterations) {
      ...
      const toolResults = await Promise.all(
        toolCalls.map(async (block) => {
          ...
          let apiName = block.input?.api_name;
          let adapter = null;
          let credentials = null;
          
          // Auto-detect API based on path if api_name not specified
          if (!apiName && block.input?.path) {
            const path = block.input.path;
            // Google Calendar paths
            if (path.includes('/calendars/') || path.includes('/calendar/v3/')) {
              apiName = 'google_calendar';
            }
            // Attio paths
            else if (path.includes('/v2/objects/') || path.includes('/v2/lists/') || path.includes('/v2/tasks/') || path.includes('/v2/notes/')) {
              apiName = 'attio';
            }
          }
