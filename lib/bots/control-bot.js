 ...
 */
async function analyzeQueryWithClaude(message, availableAdapters, claudeApiKey = null) {
...
- If query is ambiguous, include all potentially relevant tools`;
  ...
  let currentKey = claudeApiKey || getCurrentKey();
  const maxRetries = 3; // Try up to 3 keys
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.error?.type === 'rate_limit_error' || errorData.error?.type === 'overloaded_error') {
          ...
          if (nextKey && attempt < maxRetries - 1) {
            console.log(`⏳ Control bot analysis rate limited, rotating to next API key...`);
            currentKey = nextKey;
      ...
      if (attempt === maxRetries - 1) {
        console.error('Query analysis failed after all retries, using fallback:', error);
      }
    }
  }
...
function getToolDescription(toolName) {
  const descriptions = {
    attio: 'CRM system for managing contacts, companies, deals, and business relationships. Can provide email addresses, company information, and contact details for scheduling meetings.',
    google_calendar: 'Calendar system for scheduling meetings, events, and appointments. Can create events with attendees, check availability, and manage schedules. Works seamlessly with Attio to schedule meetings with contacts.',
...
- **Common pattern**: Get data from one tool (e.g., email from Attio), then use it in another tool (e.g., create calendar event with that email)`;
    } else {
      strategyGuidance = `
